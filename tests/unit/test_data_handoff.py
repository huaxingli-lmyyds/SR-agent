...
from agent.data_processing.handoff import (
    build_data_handoff,
...
def test_handoff_builder_keeps_stable_public_fields() -> None:
    handoff = build_data_handoff(
        {
            ...
            "quality_decision": {
                ...
                "blockers": [],
                "warnings": ["duration_out_of_range"],
            },
        },
        "dp_1",
    )
    ...
    assert handoff["quality_status"] == "warn"
    assert handoff["warnings"] == ["duration_out_of_range"]

def test_non_consumable_processing_output_cannot_silently_fall_back(
    tmp_path: Path,
) -> None:
...
def test_successful_data_processing_must_publish_handoff(
    tmp_path: Path,
) -> None:
    context = {
        "previous_results": {
...
def test_failed_data_processing_blocks_downstream_training(
    tmp_path: Path,
) -> None:
...
def test_quality_gate_explicitly_blocks_hpo(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="quality gate blocked"):
        resolve_data_handoff(
            _context(
                {
                    "consumer_uri": str(dataset),
                    "consumption_status": "ready",
                    "quality_status": "block",
                    "training_allowed": False,
                    "blockers": ["speaker_split_overlap"],
                }
            ),
            tmp_path / "source",
        )
