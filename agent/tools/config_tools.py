...
@tool
def UpdateConfig(config_json: str, persist: bool = True, create_backup: bool = True) -> str:
    """
...
def ListConfigParameters(path: Optional[str] = None) -> str:
    ...
    """
    try:
        config_path = Path(path) if path else Path(CONFIG_PATH)
