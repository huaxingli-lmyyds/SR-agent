...
> 数据处理结果会被关联到协调上下文和 `linked_experiments`，但当前 HPO 训练实际使用的数据路径仍来自训练配置中的 `data_folder`。运行时必须检查数据处理发布版本是否确实对应同一数据目录；当前代码不会自动把 `dataset_version.json` 的路径改写进 HPO 训练请求。
